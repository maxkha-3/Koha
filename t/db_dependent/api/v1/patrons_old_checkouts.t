...
subtest 'Patron old checkouts list() tests' => sub {
    ...
    $t->get_ok( "//$userid:$password@/api/v1/patrons/" . $patron->id . '/old_checkouts' )->status_is(200)->json_is( [] );
    ...
    $t->get_ok( "//$userid:$password@/api/v1/patrons/" . $patron->id . '/old_checkouts' )
        ->status_is(200)
        ->json_is( [] );

    # Move the checkouts to old_issues.
    C4::Circulation::AddReturn( $item1->barcode, $patron->branchcode );
    C4::Circulation::AddReturn( $item2->barcode, $patron->branchcode );
    ...
    $t->get_ok( "//$userid:$password@/api/v1/patrons/" . $non_existent_patron_id . '/old_checkouts' )
        ->status_is(404)
    ...
    $t->get_ok( "//$unauthorized_userid:$unauthorized_password@/api/v1/patrons/" . $patron->id . '/old_checkouts' )
        ->status_is(403)
